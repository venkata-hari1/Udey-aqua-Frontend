...
const PondFarming=({id,accordianId,Accordiantitle,Section='PondFarming'}:RASProps)=>{
    ...
    const SaveData = ()=>{
        ...
        console.log(Data);
    localStorage.setItem("PondFarming", JSON.stringify(Data));
    setIsSaved(true);
     setPrevData(true);
     setEdit(false)
     if (cancel){
    ...
    const CancelData = ()=>{
        const PrevData=localStorage.getItem('PondFarming');
        if (PrevData) {
    ...
    useEffect(() => {
        const saved = localStorage.getItem("PondFarming");
        if (saved) {
        setPrevData(true);
        setIsSaved(true);
        }
    ...
    return(
        <>
            <Box className={classes.WhoWeAreContainer}>
                ...
                <SubSection id='Sub Section-1' accordianId='4' title={Accordiantitle}/>
                {subpages.map((sub) => (
